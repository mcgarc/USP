...
class WireSegment:
    ...
    def field(self, t, r):
        ...
        w = self.wire_vector
        w_mag = utils.three_vector_norm(w)
        r_start = r - self.start # wire start to r
        r_start_mag = utils.three_vector_norm(r_start)
        r_end = r - self.end # wire end to r
        r_end_mag = utils.three_vector_norm(r_end)
        # Find the cosines
        # TODO Abs?
        cos_start = utils.three_vector_dot(r_start, w) / (r_start_mag * w_mag)
        cos_end = utils.three_vector_dot(r_end, w) / (r_end_mag * w_mag)
        # Find closest point on wire to r
        m = self.start + w * r_start_mag * cos_start / w_mag
        # Find R, smallest vector from wire to r
        R = r - m
        R_mag = utils.three_vector_norm(R)
        # Find direction of B field
        B_dir = utils.three_vector_cross(w, R) / (R_mag * w_mag)
        # Magnitude of B field
        B_mag = consts.u_0 * current * (cos_end + cos_start) / (4*R_mag)
        return B_dir * B_mag
